...
active_join_poll_time = 20
waiting_poll_time = 150
...
async def handle_message(message: str, author: str, twitch_message: TwitchMessage = None, youtube_message = None):
    ...
    global YOUTUBE_TIMEOUT
    global YOUTUBE_TIMEOUT_ACTIVE
    global YOUTUBE_TIMEOUT_DEFAULT
    ...
    if message.lower().startswith("!commands"):
        ...
        if is_mod:
            commands_message += " // Mod Commands: !start !clearentries !ytenable !ytdisable"
        await print_everywhere(commands_message, twitch_message=twitch_message)

    if message.lower().startswith("!race") or message.lower().startswith("!play") or message.lower().startswith("!enter") or message.lower().startswith("!join") or message.startswith("artmannJudy") or message.startswith("x100pr3Mychair") or message.startswith("x2beerShrek") or message.startswith("avoidr3Hotdogman") or message.startswith("spacec122GoodVibes") or message.startswith("artmannNatmar") or message.lower().startswith("artmannbee"):
...
async def listen_to_youtube():
    ...
    while True:
        ...
        if "items" in response is not None:
        ...
        global YOUTUBE_TIMEOUT
        await asyncio.sleep(YOUTUBE_TIMEOUT)
